...
namespace Code_Ruins.Views
{
    public partial class Settings : UserControl
    {
        ...
        public Settings()
        {
            ...
            commandReturns = new()
            {
                ["HELP"] = new Dictionary<string, CmdCommandInformation>()
                {
                    [""] = new CmdCommandInformation(
                   "有关某个命令的详细信息，请键入 HELP 命令名\nSPEED 修改玩家移动速度\nCHATTINGSPEED 聊天框输出文本速度\nIMGQ 修改游戏内图像质量\nQUIT 退出命令行",
                       null
                   ),
                    ["SPEED"] = new CmdCommandInformation(
                   "SPEED 修改玩家移动速度\nSPEED [speed] speed=>INTEGER\nDEFAULT = 3",
                       null
                   ),
                    ["CHATTINGSPEED"] = new CmdCommandInformation(
                       "CHATTINGSPEED 修改对话框输出速度\nCHATTINGSPEED [speed] speed=>INTEGER\nDEFAULT = 100",
                           null
                       ),
                    ["FPS"] = new CmdCommandInformation(
                        "FPS 显示进程帧率\nFPS [isEnable] isEnable=>BOOLEAN\nDEFAULT = false",
                        null
                        ),
                    ["QUIT"] = new CmdCommandInformation(
                   "QUIT 退出命令行\nQUIT [arg] arg=>ENUM.ELEMENT\n DEFAULT=-U\n-c : clear 清除命令台并退出;-u : usual普通退出",
                       null
                   )
                },
                ["CHATTINGSPEED"] = new Dictionary<string, CmdCommandInformation>()
                {
                    [""] = new CmdCommandInformation(
                       """
                ...
                ["FPS"] = new Dictionary<string, CmdCommandInformation>()
                {
                    ["TRUE"] = new CmdCommandInformation(
                       """
                       Now Enable FpsShower
                       """,
                       ShowFPS
                       ),
                    ["FALSE"] = new CmdCommandInformation(
                       """
                ...
                ["SPEED"] = new Dictionary<string, CmdCommandInformation>()
                {
                    [""] = new CmdCommandInformation(
                       """
                ...
                ["QUIT"] = new Dictionary<string, CmdCommandInformation>()
                {
                    ["-C"] = new CmdCommandInformation(
                       "Already Clear and Quit",
                       ReturnHomePageAndClear
                       ),
                    ["-U"] = new CmdCommandInformation(
                       "Already Quit",
                       ReturnHomePage
                       ),
                    [""] = new CmdCommandInformation(
                ...
                ["C#"] = new Dictionary<string, CmdCommandInformation>()
                {
                    [""] = new CmdCommandInformation(
                   """
                The Zen of CSharp<T>;
